...
function getImageViaProxy(url){
  return new Promise(function(resolve,reject){
    $.getImageData({
      url: url, 
      success: function(image){resolve(image)},
      server: "http://localhost:3000/"
    });
  })
}

function getImageDataViaCanvas(image){
  return new Promise(function(resolve, reject){
    ...
    var imageData = ctx.getImageData(0, 0, img_height, img_width);
    resolve(imageData)
  })
}
...
function getPredictionFromUrl(url, imgIndex){
  var predictionPromise = new Promise(function(predictionResolve, predictionReject){
    var imageProxyData = getImageViaProxy(url);
    imageProxyData.then((imageData)=>{
      console.log("got image data uri: ", imageData);
      var imageCanvasData = getImageDataViaCanvas(imageData);
      imageCanvasData.then((imageDataURI)=>{
        ...
        model.predict(inputData).then(out=>{
            ...
            predictionResolve([out.output, imgIndex]);
        });
      });
    });
  });
